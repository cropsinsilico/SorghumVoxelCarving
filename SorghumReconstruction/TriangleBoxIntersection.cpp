...
// This code is from jflipts on Github
...
/**
 * \brief Convert a QVector3D to a glm::vec3
 * \param vector A 3D vector in Qt format
 * \return A 3D vector in glm format
 */
glm::vec3 toGlm(const QVector3D& vector)
{
	return glm::vec3(
		vector.x(),
		vector.y(),
...
// This function is a wrapper around triBoxOverlap, which is using glm
	...
	const QVector3D& triangleVertex3)
{
	return triBoxOverlap(
		toGlm(boxCenter),
		toGlm(boxHalfSize),
		toGlm(triangleVertex1),
		toGlm(triangleVertex2),
		toGlm(triangleVertex3)
	);
}
